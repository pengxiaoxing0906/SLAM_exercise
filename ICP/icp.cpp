...
int main()
{
    ...
    vector<Point3f> pts_new, pts_groundtruth;//两组3D点集
    ...
    Eigen::Matrix3d rotate=rotation_vector.toRotationMatrix();
    ...
    string line;
    while(getline(textFile, line)) //getline会生成一个包含一串从输入流读入的字符的字符串，直到以下情况发生会导致生成的此字符串结束。
